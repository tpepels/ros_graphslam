...
GraphSlam::GraphSlam(ros::NodeHandle& nh) {
	...
	pose_Sub = nh.subscribe("last_pose", 1, &GraphSlam::pose_callback, this);
	//
...
void GraphSlam::pose_callback(const PoseStamped& msg){
	float new_x = msg.pose.position.x, new_y = msg.pose.position.y;
	float new_theta = tf::getYaw(msg.pose.orientation);
